...
namespace Keyfactor.Extensions.Orchestrator.WindowsCertStore.WinLdap
{
    public class Inventory : WinCertJobTypeBase, IInventoryJobExtension
    {
        ...
        public string ExtensionName => "WinLdapInventory";
...
#pragma warning restore CS8632 // The annotation for nullable reference types should only be used in code within a '#nullable' annotations context.
        ...
        public JobResult ProcessJob(InventoryJobConfiguration jobConfiguration, SubmitInventoryUpdate submitInventoryUpdate)
        {
            ...
            try
            {
                ...
                if (storePath != null)
                {
                    ...
                    _logger.LogTrace($"Attempting to read the AD DS (NTDS) LDAPS certificate store: {storePath}");
                    inventoryItems = QueryLdapCertificates(settings, storePath);
            ...
            catch (Exception ex)
            {
                ...
                var failureMessage = $"LDAP Inventory job failed for Store '{jobConfiguration.CertificateStoreDetails.StorePath}' on server '{jobConfiguration.CertificateStoreDetails.ClientMachine}' with error: '{ex.Message}'";
        ...
        public List<CurrentInventoryItem> QueryLdapCertificates(RemoteSettings settings, string StoreName)
        {
            ...
            using (PSHelper ps = new(settings.Protocol, settings.Port, settings.IncludePortInSPN, settings.ClientMachineName, settings.ServerUserName, settings.ServerPassword, jeaEndpoint: settings.JEAEndpointName))
            {
                ...
                // Get-KeyfactorLdapCertificates returns only the NTDS service store contents (the
                // single source of truth for this store type). Any Personal-store/NTDS-store mismatch
                // is surfaced as a PowerShell warning (forwarded to the orchestrator log), not as an
                // inventory row - see docsource/winldap.md for why inventory is intentionally scoped
                // to one location.
                results = ps.ExecutePowerShell("Get-KeyfactorLdapCertificates", parameters);

                // If there are certificates, deserialize the results and send them back to command
                if (results != null && results.Count > 0)
                {
                    var jsonResults = results[0].ToString();
                    var certInfoList = Certificate.Utilities.DeserializeCertificates<WinLdapCertificateInfo>(jsonResults);

                    foreach (WinLdapCertificateInfo cert in certInfoList)
                    {
