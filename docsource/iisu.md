...
There are several important caveats and considerations for users to be aware of when using the IISU store type:

- **WinRM Configuration:** The IISU store type relies on WinRM (Windows Remote Management) to perform remote management tasks. Proper WinRM configuration between the orchestrator server and the target IIS server is crucial. Misconfiguration may lead to connection issues or failure in executing remote commands.

- **Permissions:** Local Administrator permissions are required on the target IIS server. While some users might successfully use other accounts with granted rights, complexities associated with Group Policy, WinRM, and User Account Control can introduce unpredictable factors, making it challenging to use non-administrator accounts.
...
- **PFX Passwords:** The PFX password can be set to either Default (randomly generated) or Custom (specified during job creation). The latter requires the 'Allow Custom Password' application setting to be enabled in Keyfactor Command.

### SDK Usage

The IISU store type does not inherently require a specific SDK for its operations. Instead, it leverages native Windows functionalities and PowerShell commands to handle certificate management tasks, ensuring compatibility and ease of integration.

Overall, the IIS Bound Certificate Certificate Store Type (IISU) is a powerful tool for managing certificates on IIS servers, provided that the necessary configurations and permissions are correctly set up.
